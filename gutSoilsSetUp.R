...
# and makes new phyloseq objects. It also finds quantitative Jaccard distances for various
# subsets of gut samples (i.e. PV only, PD only, allopatry only,
# sympatry only)

# finds quantitative Jaccard for the gut samples
...
gut_r_vg_sampdat_catord_shavg <- read.csv(file = "~/Desktop/2019_biogeo_phan_GM/_All_pops/gut_r_vg_sampdat_catord_shavg.csv", row.names = 1, header = TRUE)
...
##### re-saved Aug. 5, 2023
# save(gut_r_veg_final_sampdat, all_samp_physeq_raw, psotu2veg, pssd2veg, gut.r.3507.1k, gut.r.3507.1k, gut.jacc.dist, soil.r.26336.1k, soil_shann, gut.r.phyloseq, PV.r.data, PD.r.data, SYMP.r.data, ALLO.r.data, PV.jacc.dist, PD.jacc.dist, SYMP.jacc.dist, ALLO.jacc.dist, PV.r.phyloseq, PD.r.phyloseq, file = "~/Desktop/UTK_Research/copyOf_All_pops/savedObjectsMarch27_2023/ps_rare_diss_shann_.RData")
